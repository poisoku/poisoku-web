...
import { FinalScraper } from '@/lib/finalScraper';
import { ScraperDatabase } from '@/lib/scraperDb';

// 最終版スクレイピングAPI（調査結果に基づく最適化済み）
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🎯 ${site}の最終版スクレイピング開始...`);
    console.log('   包括的調査結果に基づく最適化済みURL群とセレクタを使用');
    ...
    try {
      ...
      if (site === 'モッピー') {
        result = await scraper.scrapeAllMoppyFinal();
      } else {
        throw new Error(`サイト ${site} の最終版スクレイピングはまだサポートされていません`);
      }
    ...
    if (!result.success) {
      ...
      console.error('最終版スクレイピング失敗詳細:', result);
      throw new Error(`最終版スクレイピングに失敗しました: ${errorMessage}`);
    }

    console.log(`📊 最終版スクレイピング結果: ${result.campaigns.length.toLocaleString()}件の案件を取得`);
    ...
    for (let i = 0; i < campaignData.length; i += batchSize) {
      ...
      try {
        ...
        const saveResult = await scraperDb.saveCampaigns(saveResults, 'final-scrape');
    ...
    const response = {
      ...
      stats: {
        ...
        uniqueCampaigns: result.stats.uniqueCampaigns,
      ...
      breakthrough: {
        ...
        targetReached: result.stats.totalCampaigns >= 1500,
    ...
    console.log('✅ 最終版スクレイピング完了:', {
      ...
      duplicatesRemoved: response.stats.duplicatesRemoved
    });
...
function calculateEfficiency(actualCampaigns: number): number {
  const targetCampaigns = 1500; // 最終版の目標
...
function generateBreakthroughMessage(campaignCount: number): string {
  if (campaignCount >= 2000) {
    return '🎉 大成功！最終版スクレイピングで2000件以上の案件取得を達成しました！';
