...
import { PatientScraper } from '@/lib/patientScraper';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🎯 ${site}の忍耐強いスクレイピング開始...`);
    console.log('   JavaScript読み込み完了まで十分に待機し、段階的コンテンツ監視を実行');

    const scraper = new PatientScraper();
    ...
    try {
      ...
      if (site === 'モッピー') {
        result = await scraper.scrapeAllMoppyPatient();
      } else {
        throw new Error(`サイト ${site} の忍耐強いスクレイピングはまだサポートされていません`);
      }
    ...
    if (!result.success) {
      ...
      console.error('忍耐強いスクレイピング失敗詳細:', result);
      throw new Error(`忍耐強いスクレイピングに失敗しました: ${errorMessage}`);
    }

    console.log(`📊 忍耐強いスクレイピング結果: ${result.campaigns.length.toLocaleString()}件の案件を取得`);
    ...
    for (let i = 0; i < campaignData.length; i += batchSize) {
      ...
      try {
        ...
        const saveResult = await scraperDb.saveCampaigns(saveResults, 'patient-scrape');
    ...
    const response = {
      ...
      debug: {
        ...
        waitTimes: result.debug.waitTimes,
        contentLoadStages: result.debug.contentLoadStages,
      ...
      breakthrough: {
        ...
        targetReached: result.stats.totalCampaigns >= 800,
        message: generateBreakthroughMessage(result.stats.totalCampaigns)
      }
    };

    console.log('✅ 忍耐強いスクレイピング完了:', {
...
function calculateEfficiency(actualCampaigns: number): number {
  const targetCampaigns = 1500; // 忍耐強いスクレイピングの目標
...
function generateBreakthroughMessage(campaignCount: number): string {
  if (campaignCount >= 1500) {
    return '🎉 大成功！忍耐が実り1500件以上の案件取得を達成しました！';
  } else if (campaignCount >= 1000) {
    return '✅ 成功！1000件以上の案件を取得しました！';
  } else if (campaignCount >= 800) {
    return '📈 良好！800件以上の案件を取得しました。';
  } else if (campaignCount >= 500) {
    return '📊 改善成功。500件以上の案件を取得しました。';
  } else {
    return '⚠️ 更なる待機時間延長が必要です。';
  }
}