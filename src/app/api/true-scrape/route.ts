...
import { TrueScraper } from '@/lib/trueScraper';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🎯 ${site}の真の全案件スクレイピング開始...`);
    console.log('   深層調査結果に基づき、6,067件の案件取得を目指します');

    const scraper = new TrueScraper();
    ...
    try {
      ...
      if (site === 'モッピー') {
        result = await scraper.scrapeAllMoppyTrue();
      } else {
        throw new Error(`サイト ${site} の真のスクレイピングはまだサポートされていません`);
      }
    ...
    if (!result.success) {
      ...
      console.error('真のスクレイピング失敗詳細:', result);
      throw new Error(`真のスクレイピングに失敗しました: ${errorMessage}`);
    }

    console.log(`📊 真のスクレイピング結果: ${result.campaigns.length.toLocaleString()}件の案件を取得`);
    ...
    for (let i = 0; i < campaignData.length; i += batchSize) {
      ...
      try {
        ...
        const saveResult = await scraperDb.saveCampaigns(saveResults, 'true-scrape');
    ...
    console.error('真の全案件スクレイピングエラー:', error);
...
// 真の効率性計算
function calculateTrueEfficiency(actualCampaigns: number): number {
  const targetCampaigns = 6067; // 調査で発見された総案件数
...
function generateBreakthroughMessage(campaignCount: number): string {
  if (campaignCount >= 5000) {
    ...
    return '✅ 大幅突破成功！1000件以上の案件を取得しました！';
  } else if (campaignCount >= 500) {
    return '📈 中程度突破！500件以上の案件を取得しました。';
  } else if (campaignCount >= 100) {
    return '📊 小幅突破。100件以上の案件を取得しました。';
  } else {
