...
class ChobirichScraper {
  ...
  // カテゴリページから案件IDを収集（正しいページネーション対応）
  ...
  async scrapeAll() {
    const categories = [
      ...
      // 他のカテゴリーも追加できます
      // { name: 'サービス', url: 'https://www.chobirich.com/service/serv/101' },
      // { name: 'クレジットカード', url: 'https://www.chobirich.com/creditcard/card/101' }
    ];
    ...
    for (const category of categories) {
      ...
      await this.delay(5000); // カテゴリー間の待機
    }
  ...
  showStats() {
    ...
    // サンプルデータを表示
    console.log('\n=== サンプルデータ ===');
    Array.from(this.campaigns.values()).slice(0, 5).forEach(campaign => {
