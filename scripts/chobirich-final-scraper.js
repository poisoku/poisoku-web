...
class ChobirichFinalScraper {
  ...
  async init() {
    console.log('ちょびリッチ最終版スクレイパー起動中...\n');
        ...
        const findCashback = () => {
          ...
          mainContent.querySelectorAll('*').forEach(el => {
            ...
            if (/^\d+(?:\.\d+)?%$/.test(text) || /^\d+(?:,\d+)?(?:ちょび)?pt$/.test(text)) {
              ...
              const isInMainContent = mainContent.contains(el);
          ...
          // 優先順位付きフィルタリング
          const priorityFilter = (elements) => {
            // 1. AdDetailsクラスを含む要素を最優先
            let filtered = elements.filter(item => item.isAdDetails);
            if (filtered.length > 0) {
              return filtered.sort((a, b) => b.fontSize - a.fontSize)[0];
            }

            // 2. サイドバー関連を除外
            filtered = elements.filter(item => 
              !item.isSideCol && !item.isMaximumPt
            );
            if (filtered.length > 0) {
              return filtered.sort((a, b) => b.fontSize - a.fontSize)[0];
            }

            // 3. Recommendも含めて考慮（サイドバーでなければOK）
            filtered = elements.filter(item => 
              !item.isSideCol && !item.isMaximumPt
            );
            if (filtered.length > 0) {
            ...
            // 4. 最後の手段：全要素からサイドバーのみ除外
            filtered = elements.filter(item => !item.isSideCol);
            if (filtered.length > 0) {
              return filtered.sort((a, b) => b.fontSize - a.fontSize)[0];
            }
          ...
          const selectedElement = priorityFilter(cashbackElements);
  ...
  async scrapeAll() {
    // テスト：問題のあった案件で確認
    console.log('=== テスト: 楽天市場とYahoo!ショッピング ===');
    await this.scrapeCampaignDetail('36796'); // 楽天市場
    await this.scrapeCampaignDetail('41250'); // Yahoo!ショッピング
...
// 実行
async function main() {
  const scraper = new ChobirichFinalScraper();
