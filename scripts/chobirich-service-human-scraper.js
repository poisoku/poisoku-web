...
// 人間らしいアクセスパターンでサービスカテゴリ全案件を取得
class HumanLikeServiceScraper {
  constructor() {
    ...
    this.checkpointFile = 'service-human-checkpoint.json';
    this.outputFile = 'chobirich-service-campaigns.json';
  ...
  async scrapePage(categoryId, pageNum) {
    ...
    try {
      ...
      const url = pageNum === 1 
        ? `${this.baseUrl}/earn/apply/${categoryId}`
        : `${this.baseUrl}/earn/apply/${categoryId}?page=${pageNum}`;
      ...
      const pageData = await page.evaluate((categoryId) => {
        ...
        // 空ページチェック（サービスカテゴリ用のメッセージ）
        const emptyPageMessages = [
          '現在、このページに掲載しているサイトが存在しません。',
          'サイトが存在しません',
          '該当するサイトがありません'
        ];
  ...
  async run() {
    ...
    // サービスカテゴリの範囲を設定（101から開始、上限は探索的に決定）
    const categories = [];
    const maxCategory = 120; // 初期上限値（必要に応じて調整）
    
    for (let i = startCategory; i <= maxCategory; i++) {
      categories.push(i);
    }
    
    console.log('🔍 サービスカテゴリ案件取得開始（人間らしいアクセスパターン）');
    console.log(`📋 探索範囲: カテゴリ${startCategory}-${maxCategory}`);
    console.log('='.repeat(60));
    
    const startTime = Date.now();
    let emptyCategories = 0;
    const maxConsecutiveEmpty = 5; // 連続5つ空カテゴリで終了
    
    for (const categoryId of categories) {
      const categoryCount = this.allCampaigns.length;
      await this.scrapeCategory(categoryId);
      const newCount = this.allCampaigns.length - categoryCount;
      
      if (newCount === 0) {
        emptyCategories++;
        console.log(`  📊 連続空カテゴリ: ${emptyCategories}回`);
        
        if (emptyCategories >= maxConsecutiveEmpty) {
          console.log(`\n🛑 連続${maxConsecutiveEmpty}個の空カテゴリのため探索終了`);
          break;
        }
      } else {
        emptyCategories = 0; // リセット
      }
      
      await this.saveCheckpoint(categoryId);
      
      // カテゴリ間の長めの待機（30-60秒のランダム）
      if (categoryId < maxCategory) {
...
// 実行
(async () => {
  const scraper = new HumanLikeServiceScraper();
  await scraper.run();
})();