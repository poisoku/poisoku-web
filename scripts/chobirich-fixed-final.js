...
class ChobirichFixedFinal {
  ...
  async init() {
    console.log('ちょびリッチ修正版（全角％対応）起動中...\n');
  ...
  async scrapeCampaignDetail(campaignId) {
    ...
    try {
      ...
      // デバッグ情報付きでログ出力
      const debugInfo = campaign.debug.original ? ` (元: "${campaign.debug.original}")` : '';
  ...
  async scrapeAll() {
    ...
    await this.scrapeCampaignDetail('1701350'); // ANAのふるさと納税（全角％）
...
// 実行
async function main() {
  const scraper = new ChobirichFixedFinal();
