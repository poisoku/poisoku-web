...
/**
 * ポイントインカム アプリ案件 AJAXページネーション方式スクレイパー
 * ネットワークトレース解析で発見したAPIエンドポイントを直接利用
 * load_category_top.php?page=2,3,4... でページネーション
 */
class PointIncomeAjaxPaginationScraper {
  constructor() {
    this.browser = null;
    this.results = {
      ios: [],
  ...
  async execute() {
    console.log('🚀 ポイントインカム AJAXページネーション方式スクレイパー開始');
    console.log('='.repeat(70));
    console.log(`📡 発見されたAPIエンドポイント: ${this.config.ajaxEndpoint}`);
    ...
    try {
      ...
      for (const category of this.config.categories) {
        ...
        try {
          ...
          this.results[os].push(...categoryCampaigns);
          this.stats[os].categoriesProcessed++;
          this.stats[os].totalCampaigns += categoryCampaigns.length;
          
          console.log(`✅ ${categoryCampaigns.length}件の案件を取得`);
  ...
  async scrapeCategoryWithAjax(page, category, os) {
    ...
    const maxEmptyPages = 3; // 3回連続空ページで終了
    ...
    try {
      ...
      while (currentPage <= this.config.maxPagesPerCategory) {
        ...
        if (ajaxCampaigns.length === 0) {
          emptyPageCount++;
          console.log(`  📭 ページ${currentPage}: 空ページ (${emptyPageCount}/${maxEmptyPages})`);
          
          if (emptyPageCount >= maxEmptyPages) {
            console.log(`  🏁 ${maxEmptyPages}回連続空ページ - カテゴリ完了`);
            break;
          }
        } else {
  ...
  async saveResults() {
    ...
    if (this.results.ios.length > 0) {
      const iosFilename = `pointincome_ios_ajax_pagination_${timestamp}.json`;
    ...
    if (this.results.android.length > 0) {
      const androidFilename = `pointincome_android_ajax_pagination_${timestamp}.json`;
    ...
    if (combinedResults.length > 0) {
      const combinedFilename = `pointincome_ajax_pagination_combined_${timestamp}.json`;
  ...
  printFinalStats() {
    ...
    if (this.stats.ios.errors.length > 0 || this.stats.android.errors.length > 0) {
      console.log(`⚠️ エラー: iOS ${this.stats.ios.errors.length}件, Android ${this.stats.android.errors.length}件`);
    }
  }
...
if (require.main === module) {
  const scraper = new PointIncomeAjaxPaginationScraper();
  scraper.execute().catch(console.error);
}

module.exports = PointIncomeAjaxPaginationScraper;